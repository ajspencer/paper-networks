...
global utilities
utilities = Util()
#ensure that these headers are set for everyone
...
class Extractor:
    ...
    def GET(self, txtfile, method):

        extractor = Extraction()
        data = ''
        #here I deleted web header information since we don't need that
        """Check if the file exists, if not return a 404"""
        if not os.path.exists(txtfile):
            return web.notfound()
        data = data + extractor.extractHeaders(txtfile)
        return xmltodict.parse(data)
        #the only thing I am going to be getting is the header, so I will just have it return after this
        #leaving the try in for now just in case I decide to add this functionality
        try:
            if method == 'text':
...
class FileHandler(Handler):
    ...
    def POST(self, pdfpath):
        ...
        try:
            ...
            if passed is False:
                web.debug('the file was not a scientific type and the paseed was false')
                web.ctx.status = '400'
                raise ValueError
                return message
            else:
